...
devicesRouter.post("/pair", requireOwner, async (req, res) => {
  ...
  // Licence enforcement — demo account only until a real gateway is live
  // (see licenceEnforcedFor). Only ACTIVE devices count (revoking frees a
  // slot; a revoked device must re-register, so re-pairing at the limit is
  // blocked too). Runs before the screen auto-creation below so a rejected
  // pairing never leaves an orphan screen.
  if (licenceEnforcedFor(req.auth!.email)) {
